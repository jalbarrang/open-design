...
| A07 | P0 | 授权后重试 | CLI 返回未登录/授权错误，用户在该 CLI 中完成登录后自动或手动重试 | 新 Run 使用 `authorize_and_retry`；重复的已登录轮询不能重复创建 Run |
...
| ID | 优先级 | 测试场景 | 前置条件与步骤 | 预期结果 |
| --- | --- | --- | --- | --- |
| E01 | P0 | CTA 曝光 | 打开带 Retry、Continue 或切 Runtime 的失败卡 | 每个实际展示的 CTA 各有一条 `surface_view`，不能只报动作数组 |
...
3. 点击 Retry、Continue 或切 Runtime。
4. 第二个 Mock Run 成功。
...
- `e2e/ui/real-daemon-run.test.ts`：真实 UI 失败卡、Retry、目标 Run 和成功内容恢复链路。
