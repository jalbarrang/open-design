...
export async function startServer({
  ...
  console.info(
    ...
    describeRunTelemetrySink(readRunTelemetrySinkConfig(process.env)),
  );
  const design = {
  ...
  function collectBundledScenarios() {
    type ScenarioEntry = {
      id: string;
      taskKind: 'new-generation' | 'figma-migration' | 'code-migration';
    ...
    try {
      ...
      for (const row of all) {
        ...
        if (taskKind !== 'new-generation' && taskKind !== 'figma-migration' &&
            taskKind !== 'code-migration') continue;
  ...
  /**
   * Resolve an installed plugin by its exact recorded source. Local identity
   * resolution only — the catalogue itself owns availability.
   */
  ...
  const startChatRun = async (chatBody, run) => {
    ...
    // agentLaunch and resolvedBin were resolved above the resume guard.
    ...
    // reconciler to skip artifacts whose mtime predates this run.
     ...
     * report an age of a few hundred milliseconds and send diagnosis toward
    ...
    child.on('close', async (code, signal) => {
      ...
      // short-circuits. Adapters report missing session targets differently.
      // Without this branch, the failure would be swallowed by the
      ...
      // ACP session/load adapters report a durable upstream handle from the
      // ACP session. Persist it under the resume-identity guard so
  ...
  registerRunRoutes(app, {
    ...
    plugins: {
      ...
      authorizePluginRequest: async (_req, res, pluginId) => {
        if (getInstalledPlugin(db, pluginId)) return true;
