...
/** Resolve the sink used for task-level telemetry. */
...
export function describeRunTelemetrySink(
  ...
  const rawUrl = sink.kind === 'relay' ? sink.relayUrl : sink.baseUrl;
  try {
 ...
 * this helper owns only the relay/direct wire behavior. Keeping the sink
 * routing here prevents the rollout service from learning credentials
