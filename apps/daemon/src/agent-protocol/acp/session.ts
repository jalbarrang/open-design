...
export function attachAcpSession({
  ...
  // or session/load. The caller stores it per conversation to resume next turn.
  ...
  // prompt flush for still-open tools. Think-only tools are not transcribed.
  ...
  const emitTerminalToolPair = (
    ...
    st.emitted = true;
    // Think/reason frames are not concrete tool events and stay out of the
    // tool_use/tool_result transcript.
  ...
  const parser = createJsonLineStream((raw, rawLine) => {
    ...
    if (rpcErr) {
      ...
      if (finished) return;
      // JSON-RPC error handling. Unexpected-id errors are cleanup noise.
      // Model-selection failures recover through the agent's implicit default.
      if (
    ...
    if (promptRequestId !== null && obj.id === promptRequestId) {
      // Some ACP agents omit a terminal tool update. Flush any open tools
      // before completing the prompt so the transcript remains balanced.
      flushOpenAcpTools();
  ...
  return {
     ...
     * id has already been established, then closes stdin so the agent can
     * tear down its runtime. Idempotent, subsequent calls are no-ops.
     */
    abort() {
      if (aborted || finished) return;
      ...
      // Always close stdin so the agent receives EOF. This also covers aborts
      // during ACP startup, before session/new returns.
      try {
        child.stdin.end();
      } catch {
