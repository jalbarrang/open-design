...
// Resolve the `runtime_type` stamped on daemon-emitted run events. The web
// client may provide a valid BYOK hint because provider keys stay client-side;
// otherwise the daemon uses its local runtime classification.
...
//     uncached = input - read.
//   - ADDITIVE (Anthropic and some Responses-API / ACP usage): input_tokens is
//     the UNCACHED remainder and cache-read/creation tokens are reported separately on top → effective =
...
// `first_call_cache_hit_ratio` can exceed 1.0 and `uncached_input_tokens` collapses to 0.
//
