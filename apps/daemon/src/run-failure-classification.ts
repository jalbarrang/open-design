...
function isHardQuotaText(text: string): boolean {
  ...
  //
  // Balance wording sits here with the credit phrasings: being out of money is
  // exhaustion, not throttling. Without it `insufficient balance` fell through
  // to the 429 branch and came back RETRYABLE, so the daemon re-ran a job that
  // could not succeed until the user topped up.
  //
  ...
  return /\b(session limit|usage limit|limit reached|quota exceeded|quota reached|exceeded your current quota|billing (?:hard )?limit|insufficient[ _-]?(?:quota|credit|credits|funds|balance)|balance is insufficient|out of credits|no payment method|requires more credits|can only afford)\b|DAILY_LIMIT_EXCEEDED|RESOURCE_EXHAUSTED|用户额度不足|额度不足|余额不足|预扣费额度失败/i
    .test(text);
}

// A transient, retryable rate limit (distinct from a hard quota). Some providers
// return this in Chinese ("速率限制" / "请求频率"), which the English-only
 ...
 *
 * Some ACP adapters wrap a managed OpenCode startup failure before answering
 * `session/new` or `session/load`. The text describes a child runtime that
 * never came up, such as a port collision, an OOM kill, invalid config, or a
 * missing binary.
 *
...
// - Windows STATUS_ILLEGAL_INSTRUCTION (hex 0xC000001D or decimal
//   3221225501), but only inside a managed OpenCode startup wrapper. The raw
//   status code is a generic Windows SIGILL that any agent binary could emit.
...
function classifyRunFailureBase(
  ...
  if (isTimeoutText(text) || errorCode === 'TIMEOUT') {
    ...
    // from a user interrupt, which previously hid a long-running stall
  ...
  // readiness, an ACP adapter may surface a fatal and the close reason alone would
  ...
  //   STARTED. Managed ACP runtimes report their bundled
