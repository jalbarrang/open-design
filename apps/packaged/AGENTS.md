...
- Keep the packaged Vite static tree web-sidecar-owned; do not put web output under `OD_RESOURCE_ROOT`.
- `OD_RESOURCE_ROOT` is for daemon read-only resources. Keep its bundled-tree contract aligned with the authoritative list in `tools/pack/src/resources/index.ts` instead of maintaining a shorter copy here.
