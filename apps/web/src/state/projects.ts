...
import { boundedRequestErrorCode } from '../analytics/request-error-code';
import type {
  ChatMessage,
  Conversation,
  OpenTabsState,
...
export async function listProjects(options?: {
  ...
  // One coalesced read of the daemon's local project list.
  try {
...
/** Drop the coalesced local project list so the next read reaches the daemon. */
export function invalidateProjectLists(): void {
...
/**
 * Read the project row behind a deep link in one coalesced request, mapping a
 * missing row and a transport failure to distinct outcomes so the router can
 * tell "no such project" from "daemon briefly unreachable".
 */
...
export async function createProject(
  ...
  try {
    ...
    // the daemon rejects before any row is inserted, so replaying the same
...
// Warm snapshot of the visible plugin catalogue, kept so Home's remount does
// not stall on `/api/plugins` for 1-2s. A display cache only: mutations still
// read through the daemon.
const cachedVisiblePlugins = new Map<string, CachedVisiblePlugins>();
// Every request start and explicit invalidation advances the generation, so an
// older `listPlugins()` resolving late cannot put its stale snapshot back.
...
/**
 * Return the last successful plugin catalog regardless of its refresh age. Frequently remounted surfaces use this snapshot for their first render
...
/** Evict the warm plugin catalogue after a plugin install/remove/update. */
...
export async function installGeneratedPluginFolder(
  ...
  try {
    ...
    if (outcome.ok) {
      ...
      // warm catalogue first so a later mount cannot reuse the pre-install
      // snapshot for the full TTL.
