...
import type { ProjectRenameFenceToken } from '../../src/components/ProjectView';
...
vi.mock('../../src/components/ProjectView', () => ({
  ProjectView: ({
    ...
    <main data-testid="project-view">
      ...
      <button
        type="button"
        onClick={() => void onDuplicateProject?.(project.id, { name: 'Scoped duplicate' })}
      >
        Duplicate project
      </button>
      <button type="button" onClick={onOpenSettings}>Open settings from project</button>
      <button type="button" onClick={() => void onProjectsRefresh()}>Refresh projects</button>
      <button
        type="button"
        onClick={() => {
          const optimistic = {
...
describe('App project creation routing', () => {
  ...
  it('opens an unbound local project directly from the local project list', async () => {
    mockedListProjects.mockResolvedValue([{
      id: 'project-own',
    ...
    await waitFor(() => {
      expect(screen.getByTestId('project-title').textContent).toBe('Legacy local name');
    });
  ...
  it('keeps a newly created renamed project when the project route restores an older all snapshot', async () => {
    ...
    // `recent` projection.
    writeProjectDisplaySnapshot({ view: 'all' }, olderProjects);
