 ...
 * `runs/<id>/events.jsonl`, agent CLI logs, sidecar daemon/web/desktop logs,
 * and host crash reports. The desktop main
 ...
 * `OD_DATA_DIR` override, silently dropping the run-event and agent logs that
 * are the whole point of a support bundle. Delegating to the daemon endpoint removes
 * that drift entirely.
 */
