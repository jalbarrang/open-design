...
/**
 * Collect native logs from supported coding-agent CLIs. These complement the
 * per-run `events.jsonl` capture with internal diagnostics that never reach the
 * daemon's stream.
 *
 * Secret-bearing files are excluded. Only known `*.log` directories are swept,
 * and collected text still passes through the bundle's redaction pass.
 */
export async function buildAgentCliLogSources(options: AgentCliLogOptions): Promise<LogSource[]> {
  ...
  // Effective CLI homes honor app-config overrides and fall back to defaults.
