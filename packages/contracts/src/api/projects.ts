...
export type DeployProviderId = 'vercel-self' | 'cloudflare-pages' | 's3-compatible';
...
/**
 * Non-secret settings for the `s3-compatible` provider. One provider covers
 * Amazon S3, Cloudflare R2, MinIO, Backblaze B2, and DigitalOcean Spaces —
 * they differ only by `endpoint` (and `forcePathStyle` for self-hosted MinIO),
 * so there is no reason to model each vendor separately.
 *
 * The secret access key travels through `UpdateDeployConfigRequest.token` and
 * is never returned; `DeployConfigResponse.tokenMask` reports whether one is
 * stored, matching the Vercel and Cloudflare providers.
 */
export interface S3CompatibleConfigHints {
  /** Bucket-addressable API endpoint, e.g. `https://s3.us-east-1.amazonaws.com`. */
  endpoint?: string;
  /** Signing region. Cloudflare R2 expects `auto`. */
  region?: string;
  bucket?: string;
  /** Key prefix for uploaded files. Defaults to `od/`. */
  prefix?: string;
  /**
   * Base URL the published file is reachable at — a CDN or bucket website
   * endpoint. When empty the URL is derived from `endpoint` and `bucket`,
   * which only works on buckets that serve objects publicly.
   */
  publicBaseUrl?: string;
  /** Address the bucket as a path segment instead of a subdomain (MinIO). */
...
export interface DeployConfigResponse {
  ...
  projectName?: string;
  cloudflarePages?: CloudflarePagesConfigHints;
  s3?: S3CompatibleConfigHints;
  /** Access key id for `s3-compatible`. Not a secret, so it is echoed back. */
  accessKeyId?: string;
