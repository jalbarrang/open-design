...
// The surface a fresh install comes up on: EntryShell's onboarding shell
// wrapping the onboarding setup view.
const ONBOARDING_LANDING: readonly FixtureNode[] = [
...
describe('packaged app-shell probe', () => {
  it('accepts onboarding routes as packaged renderer URLs', () => {
  ...
  it('ships a self-contained expression that reads the globals the renderer has', () => {
    ...
    expect(packagedAppShellExpression).toContain('.onboarding-view');
  });

  it('tracks the setup landing rendered by the current onboarding shell', async () => {
    ...
    const landingProbe = "querySelector('.onboarding-view')";

    expect(entryShellSource).toContain('className="onboarding-view"');
    expect(packagedAppShellExpression).toContain('.onboarding-view');
    expect(macSpecSource).toContain(landingProbe);
    expect(winSpecSource).toContain(landingProbe);
  });

  it('reports home for the main shell', () => {
    expect(probe(renderFixture(HOME_SHELL))).toMatchObject({
      onboardingLandingVisible: false,
      homeVisible: true,
      onboardingVisible: false,
    });
  });

  it('reports the onboarding setup landing', () => {
    expect(probe(renderFixture(ONBOARDING_LANDING))).toMatchObject({
      onboardingLandingVisible: true,
      homeVisible: false,
...
describe('packaged app-shell terminal state', () => {
  ...
  // The bug this file exists for. A packaged first run comes to rest on the
  // onboarding setup landing — `connectStepRuntimeReady` in EntryShell.tsx will
  // not advance without an installed local CLI or a verified BYOK key, neither
  // of which a release runner has. Treating that as "not settled" made the
  // Windows smoke wait 45s for a home shell that can never arrive, which is why
  // the 0.18.0 stable cut had to fall back to `win_x64_smoke_mode: skip`.
  it('settles on the onboarding landing when the profile only needs a rendered surface', () => {
    const snapshot = probe(renderFixture(ONBOARDING_LANDING));
  ...
  it('rejects a blank window under every profile', () => {
    ...
    expect(packagedAppShellFailureReason(snapshot, { acceptOnboardingLanding: true })).toContain(
      'neither the home nav rail nor the onboarding setup landing rendered',
    );
  });
...
// The seed check and the surface policy stay separate so a lost seed fails with
// its own named cause rather than being absorbed as a genuine first run.
describe('packaged app-shell policy', () => {
  it('requires home for a seeded core run that retained onboarding completion', () => {
    const landing = probe(renderFixture(ONBOARDING_LANDING));
  ...
  it('requires home when the scenario does not establish first-run permission', () => {
    expect(
    ...
    expect(
      packagedAppShellPolicy({ coreProfile: false, daemonOnboardingCompleted: false, seededOnboardingCompleted: false }),
    ).toEqual({ acceptOnboardingLanding: false });
  });
  ...
  it('does not infer first-run permission from a completed daemon reading', () => {
    const landing = probe(renderFixture(ONBOARDING_LANDING));
...
describe('packaged launch scenarios', () => {
  ...
  it('settles a genuine first run on the onboarding landing', async () => {
  ...
  it('fails a retained completed user that stopped on the onboarding landing', async () => {
    const clock = virtualClock();
    const landing = probe(renderFixture(ONBOARDING_LANDING));
  ...
  it('fails a first run whose renderer never painted either surface', async () => {
    ...
    ).rejects.toThrow(/neither the home nav rail nor the onboarding setup landing rendered/);
  });
});
