...
a. PR-provided fixtures: if the diff includes 'tests/fixtures/fake-X.mjs' (or similar), the PR author wrote that stub specifically so tests can run without the real binary. The fixture lives on the host filesystem after checkout. The the PR daemon almost certainly reads an env var to point at it (look for 'process.env.FAKE_X_BIN' etc. in the diff). Container env is set ONCE at 'docker run' before this prompt starts -- you cannot change it mid-run. Emit §📎 Needs (e.g., "FAKE_X_BIN: prewire to the fixture path so the next run can test the positive path") so the maintainer (or harness on next iteration) can configure the run. Do NOT emit a concrete host path -- name the env var and its purpose only.
...
Example:
- ✅ **Local agent picker shows the Codex row when the mock CLI is on PATH**: launched app with the mocks/bin PATH overlay, opened /onboarding > Local agent, observed the Codex row + status pill render, network captured GET /api/agents 200.
...
Examples:
- \`OPENROUTER_API_KEY\`: real BYOK provider key to verify a live model response (the mock CLI unblocks UI only)
- \`FIGMA_CONNECTOR_TOKEN\`: real connector token to drive the OAuth callback path end to end
...
Examples:
- \`connector-oauth-spec.md\`: clarifies the token exchange flow not in the PR; would let me verify spec compliance
- \`expected-agent-row.png\`: visual reference for the local agent picker
...
- \`OD_MOCKS_TRACE\`: select the recorded mock trace; unblocks agent picker and run-stream rendering
