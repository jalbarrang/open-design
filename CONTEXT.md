...
**Local Agent CLI**:
The command-line coding agent OpenDesign spawns on the user's machine to run a turn. The user installs and authenticates it themselves; OpenDesign discovers it on PATH.
_Avoid_: hosted runtime, managed agent, cloud account

**BYOK Provider Key**:
A model-provider API key the user supplies so OpenDesign can call that provider directly from the local daemon.
_Avoid_: subscription, hosted credential, account status
...
- A **Local Agent CLI** and a **BYOK Provider Key** are alternative ways to give a **Project** a working model; both run from the local daemon.
