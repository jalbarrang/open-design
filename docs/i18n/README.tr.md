...
Node `~26`, pnpm `10.33.x`. Windows kullanıcıları, bkz. [`docs/windows-troubleshooting.md`](../../docs/windows-troubleshooting.md). Tam hızlı başlangıç, ortam değişkenleri, paketlenmiş derleme akışı → [`QUICKSTART.md`](../../QUICKSTART.md).
...
- [x] ~~**0.9.0** — OpenDesign Cloud (uygulamaya gömülü resmî Model Router: sıfır yapılandırma, tek tıkla oturum açma)~~ (removed — OpenDesign is local CLI + BYOK only)
