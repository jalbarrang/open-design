...
ต้องใช้ Node `~26` และ pnpm `10.33.x`. `nvm` / `fnm` เป็น optional; ใช้ `nvm install 26 && nvm use 26` หรือ `fnm install 26 && fnm use 26` ถ้าคุณชอบจัดการ Node ด้วยวิธีนั้น. macOS, Linux และ WSL2 เป็น path หลัก. Windows native รองรับด้วย; ดู gotchas การ setup ที่พบบ่อยใน [`docs/windows-troubleshooting.md`](../../docs/windows-troubleshooting.md).
