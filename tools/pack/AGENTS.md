...
- For ordinary Windows NSIS smoke tests, use short namespaces such as `rg`, `smoke`, or `nsis-a`. Namespace-scoped install roots still contain deeply nested dependency and resource paths; short namespaces avoid the traditional Windows 260-character limit. Use long namespaces only when intentionally testing installer path-length behavior.
