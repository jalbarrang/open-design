...
NODE_BASE_IMAGE="${NODE_BASE_IMAGE:-docker.io/library/node:26-alpine}"
RUNTIME_BASE_IMAGE="${RUNTIME_BASE_IMAGE:-docker.io/library/node:26-alpine}"
...
Options:
  ...
  --node_base_image <image-ref>   default: docker.io/library/node:26-alpine
  --runtime_base_image <image-ref> default: docker.io/library/node:26-alpine
...
node_local_base_image() {
  ...
  local arch
  arch="$(platform_to_arch "$platform")" || die "unsupported platform '$platform'"
  printf 'open-design-base-node:26-alpine-%s' "$arch"
}
